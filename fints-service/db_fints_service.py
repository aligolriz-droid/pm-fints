...
def _collect(client, days: int) -> Dict[str, Any]:
    ...
    try:
        # DKB sometimes rejects the very first call to get_sepa_accounts if the session 
        # isn't fully "warm" yet. We wrap this in a small retry.
        accounts = None
        for attempt in range(3):
            try:
                ...
                err = str(e)
                if ("9210" in err or "9800" in err) and attempt < 2:
                    time.sleep(2)
                    continue
                raise e
    except Exception as e:
        ...
        if "9210" in err_msg or "9800" in err_msg or "9050" in err_msg:
            raise HTTPException(status_code=400, detail=f"Bank lehnte Anfrage ab (Sitzung ungültig/abgelehnt). Bitte Login erneut starten. Details: {err_msg}")
...
def _select_tan_mechanism(client) -> None:
    """
    Mimics the local connector's bootstrap behavior.
    DKB is extremely sensitive to the chosen TAN mechanism.
    """
    try:
    ...
    # Priority 1: Decoupled/Push (The "SecureGo" way)
    chosen = None
    for sec_fn, mech in mechs.items():
        name = (getattr(mech, "name", "") or "").lower()
        # DKB often uses "decoupled" or "push" in the mechanism name
        if any(kw in name for kw in ["push", "securego", "decoupled", "app", "sca"]):
            chosen = sec_fn
            break
    
    # Priority 2: Fallback to any non-999 mechanism
    if chosen is None:
        for sec_fn in mechs:
            if str(sec_fn) != "999":
                chosen = sec_fn
...
def tan(req: TanReq):
    ...
    try:
        ...
        with client.resume_dialog(s["dialog_data"]):
            ...
            # TAN akzeptiert -> Daten einsammeln
            # Polling-Loop fuer decoupled TAN / SCA-Verzoegerung
            max_retries = 5
            for attempt in range(max_retries):
                try:
                    ...
                    # 9210/9800 = Bank ist noch nicht bereit oder Dialog tot
                    if e.status_code == 400 and any(code in str(e.detail) for code in ["9210", "9800", "9050"]):
                    ...
                    # Handle connection drops or unexpected bank resets
                    if ("Remote end closed" in str(e) or "Connection aborted" in str(e)) and attempt < max_retries - 1:
                        time.sleep(3)
                        continue
                    raise e
    except _TanNeeded as need:
