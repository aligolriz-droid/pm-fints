...
def _select_tan_mechanism(client) -> None:
    """
    Mimics the minimal_interactive_cli_bootstrap logic for cloud usage.
    Ensures a proper TAN mechanism is selected before the dialog starts.
    """
    try:
        # First, always fetch the latest mechanisms from the bank
    ...
    # Priority 1: PushTAN / SecureGo / Decoupled (The DKB way)
    chosen = None
    for sec_fn, mech in mechs.items():
        name = (getattr(mech, "name", "") or "").lower()
        if any(kw in name for kw in ["push", "securego", "decoupled", "app"]):
            chosen = sec_fn
            break
    
    # Priority 2: Any mechanism that is NOT the default "999"
    if chosen is None:
        for sec_fn in mechs:
            if str(sec_fn) != "999":
                chosen = sec_fn
        ...
        with client.resume_dialog(s["dialog_data"]):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    # If the connection was dropped (RemoteDisconnected), we might want to retry
                    if "Remote end closed" in str(e) and attempt < max_retries - 1:
                        time.sleep(3)
                        continue
                    raise e
    except _TanNeeded as need:
