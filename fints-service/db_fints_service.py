...
def tan(req: TanReq):
    ...
    try:
                ...
                try:
                    ...
                    if e.status_code == 400 and any(code in str(e.detail) for code in ["9210", "9800", "9050"]):
                        if attempt < max_retries - 1:
                            time.sleep(3) # Kurze Pause und erneut versuchen
                            continue
                        raise e
                    raise e
                except _TanNeeded as need:
                    # Wenn während des Sammelns erneut eine TAN gefordert wird
                    s["ts"] = time.time()
        ...
        # Dieser Block ist redundant wegen des inneren try, aber zur Sicherheit:
        s["ts"] = time.time()
        s["tan_blob"] = need.resp.get_data()
        return _tan_payload(req.session, need.resp)
    except HTTPException:
