...
def _collect(client, days: int) -> Dict[str, Any]:
    ...
    print(f"=== DEBUG: unmittelbar vor get_sepa_accounts(): "
          f"current={client.get_current_tan_mechanism()!r} "
          f"selected_security_function={getattr(client, 'selected_security_function', '???')!r} ===")
...
def holdings(req: HoldReq):
    ...
    print(f"=== DEBUG: TAN-Verfahren nach _select_tan_mechanism (vor with-Block): "
          f"current={client.get_current_tan_mechanism()!r} "
          f"selected_security_function={getattr(client, 'selected_security_function', '???')!r} ===")
    try:
        with client:
            print(f"=== DEBUG: TAN-Verfahren nach with-Block-Eintritt (Dialog offen): "
                  f"current={client.get_current_tan_mechanism()!r} "
                  f"selected_security_function={getattr(client, 'selected_security_function', '???')!r} "
                  f"init_tan_response={getattr(client, 'init_tan_response', None)!r} ===")
            try:
